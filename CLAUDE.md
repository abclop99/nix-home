...
- **Do not read the contrast ratio as perception.** WCAG 2.1 is luminance-only: it ignores chroma, so it overstates saturated colours (Helmholtz-Kohlrausch makes them look lighter, hence closer to a light background than the ratio claims), it is blind to polarity, and its `+0.05` floor compresses the dark end — which is why Frappe scores much better than Latte while having *smaller* luminance gaps. The harness reports OKLCH ΔL beside every ratio for this reason; against Latte's base, body text is 0.52, quiet structure 0.25, and 0.15 is effectively invisible. It was reliable for the near-neutral greys (chroma ≈0.02) and misleading for the accents (chroma ≈0.17).
