...
- `pkgs.modrinth-app` is wrapped with `jdk8/17/21/25` on its internal PATH (overridable `jdks` arg), so Java auto-detects on first run. But Modrinth's backend (`check_java_at_filepath`) `canonicalize()`s the selected Java and stores the resolved `/nix/store/…/bin/java` real path in its SQLite DB; it re-canonicalizes every launch but only validates that stored path, so pointing it at any symlink (`~/.nix-profile/bin/java`, etc.) doesn't help. The stored path goes stale only after a `nix-collect-garbage` following a JDK store-path bump — launch then fails (typically `No such file or directory`, not necessarily "Could not check Java version"). No auto-heal; fix is one click in Settings → Java installations → re-detect / "Install recommended". The package is `unfree` (`unfreeRedistributable`), already covered by `allowUnfree`.
